...
namespace Vproject
{
    public partial class StuOperations : Form
    {
        public StuOperations()
        {
            InitializeComponent();
        }
        ...
        {
            string delete = "delete from StudentRegistration where StudentId=@StudentId ";
            komut = new SqlCommand(delete, baglanti);
            komut.Parameters.AddWithValue("@StudentId", Convert.ToInt32(txtBxDelete.Text));
            baglanti.Open();
        ...
        {
            baglanti.Open();
            SqlCommand komut = new SqlCommand("Select * from StudentRegistration where StudentName like '%" + txtBxSearch.Text + "%'", baglanti);
        ...
        private void StuOperations_Load(object sender, EventArgs e)
        {
            gridgetir();
            ...
            for (int i = 0; i < dtGridOperation.Rows.Count - 1; i++)
            {
                ...
                if (Convert.ToString(dtGridOperation.Rows[i].Cells["Faculty"].Value) == "Mühendis")
                {
                    renk.BackColor = Color.YellowGreen;
                }
                else if (Convert.ToString(dtGridOperation.Rows[i].Cells["Faculty"].Value) == "Tıp")
                {
                    renk.BackColor = Color.Orange;
                }
                else if (Convert.ToString(dtGridOperation.Rows[i].Cells["Faculty"].Value) == "Sosyal Hizmetler")
                {
